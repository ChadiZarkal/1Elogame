...
const DEMO = [
  { text: 'il est drôle et attentionné', score: 10, connector: null, icon: '🟢' },
  { text: 'il est jaloux maladif', score: 6, connector: 'MAIS', icon: '🚩' },
...
export function Intro({ onStart, failed }: { onStart: () => void; failed: boolean }) {
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="scrollbar-hide flex flex-1 flex-col overflow-y-auto px-6 pb-8"
    >
      <div className="flex flex-1 flex-col items-center justify-center py-4 text-center">
        ...
        <p className="mt-5 max-w-xs text-sm font-semibold leading-snug text-white/55">
          Les révélations <span className="text-white">s&apos;empilent</span>. À chaque
          fois, tu renotes la personne{' '}
        ...
        <Link
          ...
          className="mt-5 flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.2em] transition-colors hover:text-amber-300"
      ...
      <motion.button
        whileTap={{ scale: 0.96 }}
        onClick={onStart}
        className="w-full cursor-pointer rounded-2xl py-5 text-lg font-black uppercase tracking-widest text-black"
        style={{
...
/** Boucle : une phrase s'ajoute, la note tombe. Se passe de commentaire. */
