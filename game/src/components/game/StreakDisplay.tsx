...
export function StreakDisplay({ streak, streakEmoji, duelCount }: StreakDisplayProps) {
  ...
  return (
    <div className="flex items-center gap-2 animate-streak-container">
      {streak > 0 && (
        <div
          key={`streak-${streak}`}
          className={`backdrop-blur-sm rounded-full px-3 py-1.5 flex items-center gap-1.5 animate-streak-pop ${isMilestone ? 'animate-milestone-pulse' : ''}`}
          style={{
        ...
        >
          ...
          {isMilestone && (
            <span
              className="text-xs font-black animate-fade-in-fast"
              style={{ color: '#EF4444' }}
            >
              🔥
            </span>
